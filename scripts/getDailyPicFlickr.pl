...
my $url = "https://www.flourish.org/flickr/daily-interesting-one.xml"; #Flickr photo of the day rss feed
...
my @lines2 = grep (/href=/, @lines);
@lines2 = grep (/img\.earthshots\.org.*\.jpg/, @lines2);
@lines2 = grep (/full/, @lines2);

#extract the image ID (the last number in the url)
my $imageID = $url;
$imageID =~ s#.*/([0-9]*)/$#$1#;
...
@lines2 = grep (/staticflickr\.com/, @lines2);
@lines2 = grep (/og:image/, @lines2);
@lines2 = grep (/$imageID/, @lines2);
my $img = $lines2[0];
$img =~ s#.*content="([^"]*)".*#$1#;
